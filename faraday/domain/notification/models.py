"""Domain notification — shard real (W7).

16 clases extraidas verbatim desde faraday/server/models.py:
NotificationSubscription*, NotificationEvent, NotificationBase (+3 subtipos),
Notification, BaseNotification, UserNotification, UserNotificationSettings,
EmailNotification, SlackNotification.

ExecutiveReport y EventType permanecen en faraday/server/models.py y se
re-exportan aqui para mantener el contrato del shard
(faraday/domain/README.md, notification: 12 clases).

Patron: lazy `db` import como faraday/domain/base.py para evitar ciclo
duro; Metadata viene de faraday/domain/base.py (ya real).
Relaciones cross-shard usan string references ('User', 'Role', 'Workspace',
'EventType', 'ObjectType') — la resolucion es lazy por registry SQLAlchemy.

Ciclo domain<->server: si este modulo se importa ANTES que
faraday.server.models, la importacion de `db` dispara la carga completa de
server.models, cuyo shim cae en el fallback ImportError y define las clases
localmente; al retomar este modulo las tablas ya existen en el MetaData, asi
que se detecta y se re-exportan esas clases (identidad preservada en ambos
ordenes, sin doble definicion).
"""
...
if 'notification_subscription' in db.metadata.tables:
    # Ciclo domain-first: server.models ya definio las clases via fallback.
