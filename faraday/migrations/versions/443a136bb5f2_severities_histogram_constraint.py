...
def upgrade():
    ...
    conn = op.get_bind()
    rows = conn.execute(text('SELECT '  # nosec
               ...
               'having COUNT(*) > 1'))
    for row in rows:
        conn.execute(text(f"DELETE FROM severities_histogram WHERE workspace_id={row[4]} AND date='{row[5]}'"))  # nosec
        conn.execute(text(f"insert into severities_histogram (workspace_id, date, medium, high, critical, confirmed)"
