...
def upgrade():
    ...
    res = bind.execute(text('SELECT name, id FROM event_type')).fetchall()  # nosec
    event_type_ids = dict(res)

    res = bind.execute(text('SELECT name, id FROM faraday_role')).fetchall()  # nosec
    role_ids = dict(res)
...
def downgrade():
    ...
    for config in notifications_config:
            ...
            for event_type_id in event_type_name:
                ...
                for subscription_id in subscriptions:
                    ...
                    for base_config_id in base_config:
                        session.execute(text(f"DELETE "  # nosec
        ...
        session.execute(text(f'DELETE FROM event_type WHERE name IN ({name_list})'))  # nosec
